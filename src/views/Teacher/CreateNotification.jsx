...
class CreateNotification extends React.Component {
    constructor(props) {
        ...
        this.yupSchema = object().shape({
            ...
            notificationcreateddate: string().required("Notification Start Date is required."),
        ...
        this.state = {
            adharNumber: '', updateText: '', startSpinner: false, notificationObject: '', isUpdate: '', isError: false, errorMessage: '', isSuccess: false, successMessage: '', buttonText: "Register"
        }
    }

    backDashboard = () => {
        this.setState({ isSuccess: false, isError: false })
        if (this.props.notificationid || this.props.isMobileView) {
    ...
    handleSubmit = async (values) => {
        ...
        if (this.props.notificationid == undefined) {
            response = await this.props.authenticatedApiCall('post', "/api/notificationservice/notificationRegistration", dateToSend)
        } else {
            response = await this.props.authenticatedApiCall('put', "/api/notificationservice/updatenotifications/" + this.props.notificationid, dateToSend)
        }
                    ...
                    {(props) => (
                        <Form>
                            <Paper style={{ margin: "0px", height: "70px", width: "100%" }}>
                                <Typography className={classes.center}>{this.state.updateText}Create Notification</Typography>
                            </Paper>
...
export default withStyles(styles)(AuthenticatedPage("Teacher")(CreateNotification));
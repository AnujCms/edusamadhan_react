...
import ActionButton from '../Notifications/ActionNotificationButton';
...
class TeacherNotifications extends React.Component {
    state = {
    ...
    tableheads1 = [
        ...
        },
        {
            ...
            options: {
                ...
                customBodyRender: (value) => {
                    return (
                        <ActionButton notificationid = {value.notificationid} createdby={value.createdby} role = {this.props.currentUser.userDetails.role} onEditNotification={this.handleEditNotification} onDeleteNotification={this.handleDeleteNotification}/>
                    )
                }
            }
        }
    ...
    async componentDidMount() {
        ...
        if (response.data.status == 1) {
            response.data.statusDescription.map((item) => {
                ...
                else if(item.createdby === 3){item.createdby = 'Creted by You'}
            })
            this.setState({
    ...
    handleCreateNotification = () =>{
        this.props.history.push('./createNotification')
    }
    backDashboard = () => {
            ...
            <div className={classes.root}>
                ...
                <Grid container className={classes.GridContainer}>
                    <Grid item lg={8} md={8} sm={12} xs={12} className={classes.pad0}>
                        <Typography variant="h4" className={classes.evenetsTitle}>Class Notifications</Typography>
                    </Grid>
...
export default withStyles(styles)(AuthenticatedPage("Teacher")(WithAccount(TeacherNotifications)));
...
class FeeDetails extends React.Component {
    ...
    tableheads1 = [
...

        {
            ...
            options: {
                ...
                print: false,
                customBodyRender: (value) => {
                    return (<ActionButtonForEditFeeDetails classs={value.class} mediumType={value.mediumType} onEditDetails={this.onEditDetails} />)
    ...
    //Edit Fee Details
    onEditDetails = (classs, mediumType) => {
        this.props.history.push('./edit-feedetails/' + classs + '/'+ mediumType);
    }
    ...
    async componentDidMount() {
        ...
        if (response.data.status == 1) {
            response.data.statusDescription.map((item) => {
                item.action = { class: item.class, mediumType: item.mediumType }

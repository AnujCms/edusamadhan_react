...
const styles = theme => ({
    successDialogOK: {color: theme.palette.text.textPrimaryColor, backgroundColor: theme.palette.primary.main, border: "1px solid " + theme.palette.border.primaryBorder, borderRadius: "50px", margin: "8px 0", textAlign: "right", padding: "7px 15px", '&:hover': { backgroundColor: theme.palette.hoverPrimaryColor.main, color: theme.palette.text.hoverTextPrimaryColor, border: "1px solid " + theme.palette.border.hoverPrimaryBorder }},
    successDialogCancel: {color: theme.palette.text.textThirdColor, backgroundColor: theme.palette.thirdColor.main, border: "1px solid " + theme.palette.border.thirdBorder, borderRadius: "20px", padding: "7px 15px", '&:hover': { color: theme.palette.text.hoverTextThirdColor, backgroundColor: theme.palette.hoverThirdColor.main, border: "1px solid " + theme.palette.border.hoverThirdBorder }},
...
confirmReExamText = '';
    ...
    handleReExam = async () => {
        ...
        let response1 = await this.props.authenticatedApiCall('put', '/api/examinationservice/reexamentrance', { studentId })
    ...
    handlePromote = async () => {
        let response1 = await this.props.authenticatedApiCall('put', "/api/examinationservice/pramotestudent/" + this.props.location.state.studentid + "/" + this.props.location.state.class, null)
        if (response1.data.status == 0) {
    ...
    handleDelete = async () => {
        let response1 = await this.props.authenticatedApiCall('delete', "/api/examinationservice/deletestudent/"+ this.props.location.state.studentid, null)
        if (response1.data.status == 0) {

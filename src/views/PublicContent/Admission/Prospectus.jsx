...
class Prospectus extends React.Component {
    state = { prospectusUrl: '', isRender: false, errorMessage: '' }
    componentDidMount = () => {
        let accountId = 'eac1efd0-ce2d-11ea-9b0d-21e50464571f'
        axios.get('/api/publiccontent/getPublicContentUrl/' + accountId + "/" + 2).then(response => {
            if (response.data.status === 1) {
                this.setState({ isRender: true, prospectusUrl: response.data.statusDescription[0].contentUrl })
    ...
    render() {
        ...
        const { isRender, prospectusUrl, errorMessage } = this.state;
        return (
            <div className={classes.root}>
                <Helmet> <title>Prospectus</title></Helmet>
                <Paper className={classes.formHeader}>
                    <Typography className={classes.center}>SCHOOL PROSPECTUS</Typography>
                </Paper>
...
export default (withStyles(styles)(Prospectus));
